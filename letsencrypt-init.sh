...
# rename healthlifevn.conf temporarily
if [ -e ${REPO_DIR}/nginx/healthlifevn.conf ]; then
  mv ${REPO_DIR}/nginx/healthlifevn.conf ${REPO_DIR}/nginx/healthlifevn.conf.waitforletsencrypt
fi
...
if [ -e ${REPO_DIR}/nginx/heathlifevn.conf.waitforletsencrypt ]; then
  mv ${REPO_DIR}/nginx/healthlifevn.conf.waitforletsencrypt ${REPO_DIR}/nginx/healthlifevn.conf
fi

cd ${LE_DIR}
rm -f ${REPO_DIR}/lets_encrypt.conf

echo "INFO: update the nginx/healthlifevn.conf file"
echo "-  4:   server_name healthlifevn.com;"
echo "- 19:   server_name               healthlifevn.com www.healthlifevn;"
echo "- 40:   ssl_certificate           /etc/letsencrypt/live/healthlifevn/fullchain.pem;"
echo "- 41:   ssl_certificate_key       /etc/letsencrypt/live/healthlifevn/privkey.pem;"
echo "- 42:   ssl_trusted_certificate   /etc/letsencrypt/live/healthlifevn/chain.pem;"
